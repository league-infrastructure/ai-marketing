---
name: show-work
description: Make generated images and files visible to Eric in his browser. Use IMMEDIATELY whenever an image or visual artifact is created outside an open project gallery, and whenever Eric says "show me", "open it", "I can't see it", or similar.
---

# Showing Work

Eric reviews everything in his browser. An image he cannot see does not exist.
Never end a turn having produced a visual artifact that is not on his screen.

## Decision table

| What you made | How to show it |
|---|---|
| An iteration in a project | Nothing extra needed if the gallery is already open — it auto-reloads. If in doubt, or if he says he can't see it, call `open_project(name)` again (it reuses the server). |
...
- **Verify the open worked.** `open` and `webbrowser` return success even for a wrong
  path. Echo the exact URL/path you opened; if Eric says he doesn't see it, re-open
...
- **After showing, stop and wait.** The next move is Eric's reaction.
