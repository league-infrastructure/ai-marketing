...
description: Run a League marketing content project end to end — set up the project, open the live gallery, iterate on images from chat feedback, and harvest approved art into components. Use whenever the designer asks to create a new postcard/flyer/logo/image project or to continue iterating on one.
---
...
1. **Parse the brief.** Extract: style, composition, layout, palette, theme, scene,
   ...
   orientation) unless the designer says it's only for subject detail.
2. **`create_project`** with that config, passing reference photos as `source_images`.
3. **`open_project` immediately** — before the first generation. Tell the designer the
   URL. This is mandatory: they watch the gallery, not the filesystem.
...
the designer. If they say they can't see it, use the `show-work` skill right away.
...
- Commit only when the designer says to.
...
- New style names require an MCP server restart (in-memory `STYLES` list) — ask the
  designer to restart; the `.md` prompt files themselves are re-read every call.
- Real solutions over hacks unless they say "do it right now".
