...
#
# Rendering index.html/postcard.html and serving them over HTTP is NOT this process's job —
# that all lives in webserver.py, a separate daemon process (see below). This file writes
# raw project data + generated image files to disk, then POSTs to the daemon to have it
# persist + render. That split means editing rendering/template/route code only ever
# requires restarting the daemon (restart_web_server), never this MCP server.
...
def _load_postcard_content(pdir: Path) -> dict:
    """Read-only copy of webserver.py's loader — needed here so postcard tools can inspect
    and mutate content locally before POSTing the updated dict for the daemon to persist."""
...
#
# The static/postcard web server (webserver.py) runs as its own OS process, not a thread in
# this one — so editing it never requires restarting this MCP connection. This process only
# needs to (a) make sure a daemon is running before pointing a browser or Playwright at it,
# and (b) restart it on request once its code has changed.
...
DEFAULT_STATIC_PORT = 31337  # fixed "weird" port so the gallery URL is stable across restarts
                              ...
                              # webserver.py falls back to a random free port only if this one
...
def _webserver_post(path: str, body: dict, timeout: float = 30.0) -> dict:
    """POST to the web-server daemon (spawning it first if needed) and return its parsed
    JSON response. Raises if the daemon reports an error, so callers can just let it
    propagate into a clean tool-level error message."""
...
def open_project(name: str, serve: bool = True, port: int = 0) -> str:
    ...
    With serve=True (default) the web server daemon (rooted at the projects/ directory, so
    URLs are http://127.0.0.1:<port>/<slug>/...) is started if not already running, and the
    browser opens this project's page on it. With serve=False the index.html is opened
    directly via file:// (auto-reload falls back to a periodic refresh). `port` only takes
    effect the first time the daemon starts; once running, every project (and
    open_projects_home) reuses the same port."""
    data = _load_project(name)
    if not data:
        return json.dumps({"error": f"No project named '{name}'"})
...
def open_projects_home(port: int = 0) -> str:
    ...
    web server daemon as open_project — `port` only takes effect on first boot."""
    chosen = _ensure_webserver(port)
    try:
        _webserver_post("_home/refresh", {})
    except Exception:
        pass  # best-effort — the home page is already kept fresh by every project save
...
# per region, and pressing Enter there POSTs straight to the web server daemon (webserver.py's
# do_POST), which updates postcard-content.json and bumps state.json so the open tab reloads
# with the edit — no chat round-trip needed for a wording tweak. See webserver.py's
# _render_postcard_html / templates/postcard.html.j2 for the preview page, and
# generate_postcard_pdf below for print (also just a thin call into the same daemon).
...
def generate_postcard_pdf(name: str, out_path: str = "") -> str:
    ...
    This is the same route the browser's "Generate PDF" button on postcard.html calls —
    the actual PDF-generation logic lives entirely in webserver.py.
    ...
    if not data:
        return json.dumps({"error": f"No project named '{name}'"})
    try:
        # Playwright + bleed processing can take a while — generous timeout.
...
def restart_web_server(port: int = 0) -> str:
    ...
    only changes to THIS file (server.py's own tool definitions) still need that."""
    status = _webserver_status()
    if status:
        try:
